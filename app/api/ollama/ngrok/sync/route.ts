...
export async function POST(request: NextRequest) {
  ...
  const body = (await request.json().catch(() => null)) as
    | { repository?: unknown; projectName?: unknown; branch?: unknown; targets?: unknown }
    | null;
  ...
  const port = 11434;
  const ngrok = await ensureNgrokTunnel(port);
  if (!ngrok.publicUrl) {
    return NextResponse.json(
      { ok: false, error: ngrok.error || "Failed to start ngrok", started: ngrok.started, port },
      { status: 200 }
    );
  }
  ...
  try {
    ...
    const currentEnvs = await vercel.getProjectEnvironmentVariables(projectName);
    const currentUrlVar = currentEnvs.find((e) => e.key === "OLLAMA_BASE_URL");
    const isAlreadySynced = currentUrlVar?.value === ngrok.publicUrl;
    ...
    if (isAlreadySynced) {
       return NextResponse.json({
        ...
        branch,
        projectName,
        message: "OLLAMA_BASE_URL is already up to date. No deployment needed.",
        ngrok: {
          publicUrl: ngrok.publicUrl,
          started: ngrok.started,
        },
      });
    }
    ...
    await vercel.updateProjectEnvironmentVariables(projectName, [
        {
          key: "OLLAMA_BASE_URL",
          value: ngrok.publicUrl!,
          target: targets,
        },
    ]);

    // Trigger a deployment
    const deployResult = await vercel.deployFromGitHub({
      projectName,
      repository,
      branch,
      skipEnvUpdate: true,
    ...
    return NextResponse.json({
      ...
      message: "OLLAMA_BASE_URL updated and deployment triggered.",
      ngrok: {
        publicUrl: ngrok.publicUrl,
        started: ngrok.started,
      },
    ...
    return NextResponse.json(
      {
        ok: false,
        error: error instanceof Error ? error.message : "Failed to sync ngrok to Vercel",
        ngrok: {
          publicUrl: ngrok.publicUrl,
          started: ngrok.started,
        },
