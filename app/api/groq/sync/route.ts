...
export async function POST(request: NextRequest) {
  ...
  const apiKeyFromEnv = (process.env.GROQ_API_KEY || process.env.NEXT_PUBLIC_GROQ_API_KEY || "").trim();
  const apiKey = apiKeyFromBody || apiKeyFromEnv;

  const apiKeySource = apiKeyFromBody
    ? "request"
  ...
  if (!apiKey) {
    return NextResponse.json(
      {
        ok: false,
        error: "GROQ_API_KEY is missing. Add it to .env.local (recommended) or pass { apiKey } in the request body.",
      },
      { status: 200 }
    );
  }
  ...
  const warning =
    apiKeySource === "NEXT_PUBLIC_GROQ_API_KEY"
      ? "NEXT_PUBLIC_GROQ_API_KEY is set; move it to GROQ_API_KEY to avoid exposing your key to the browser."
      : apiKeySource === "request"
        ? "API key was provided in the request body; prefer setting GROQ_API_KEY in .env.local instead."
        : null;
    ...
    return NextResponse.json(
      {
        ok: false,
        warning,
        error: error instanceof Error ? error.message : "Failed to sync Groq key to Vercel",
      },
      { status: 200 }
    );
  }
